...
export default class FeedItem extends React.Component {
  ...
  handleCommentPost(commentText) {
    ...
    postComment(this.state._id, 4, this.state.commentlikeCounter, commentText, (updatedFeedItem) => {
  ...
  */
  ...
  handleCommentLikeClick(clickEvent) {
    ...
    if (clickEvent.button === 0) {
      ...
      if (this.didUserLike()) {
        // User clicked 'unlike' button.
        unlikeCommentItem(this.state._id, 4, callbackFunction);
      } else {
        // User clicked 'like' button.
        likeCommentItem(this.state._id, 4, callbackFunction);
      }
    }
  }
  ...
  */
  ...
  render() {
    ...
    var likeButtonTextComment = "Like";
    if (this.didUserLikeComment()) {
      likeButtonTextComment = "Unlike";
    }
    switch(data.type) {
      case "statusUpdate":
            ...
            <CommentThread onPost={(commentText) => this.handleCommentPost(commentText)}>
              {
                data.comments.map((comment, i) => {
                  ...
                  return (
                    <Comment key={i} author={comment.author} postDate={comment.postDate} likeCounter={<a href="#" onClick={(e) => this.handleCommentLikeClick(e)}>
                       {likeButtonTextComment}
                      </a>}>{comment.contents}</Comment>
                  );
                })
              }
            </CommentThread>
